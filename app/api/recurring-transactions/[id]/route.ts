...
async function parseRecurringTransactionId(
  paramsPromise: RouteContext["params"]
) {
  const params = await paramsPromise
  const parsedId = recurringTransactionIdSchema.safeParse(params.id)
...
export async function GET(_: NextRequest, { params }: RouteContext) {
  ...
  const parsedId = await parseRecurringTransactionId(params)

  if (parsedId.error !== undefined) {
    return validationErrorResponse(parsedId.error)
  }

  const result = await getRecurringTransactionById(
    auth.supabase,
    auth.user.id,
    parsedId.id
  )
...
export async function PATCH(request: NextRequest, { params }: RouteContext) {
  ...
  const parsedPayload = updateRecurringTransactionSchema.safeParse(payload)
